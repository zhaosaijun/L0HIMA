...
Sim_ms<-function(Q){
  ...
  if(!is.null(HDMA.fit)){
    L0hma.fit<-HDMA.fit$contributions[which(HDMA.fit$contributions[,7]<0.05),]
    L0hma.fit[,1]<-as.numeric(substr(L0hma.fit[,1], 2, 1000))
    ID_ms_L0[L0hma.fit[,1]]=1
    me_est[L0hma.fit[,1]]=L0hma.fit[,2]*L0hma.fit[,4]
  }
...
clusterExport(cl, c('simL0hma','L0hma','mediate_hdma'))
