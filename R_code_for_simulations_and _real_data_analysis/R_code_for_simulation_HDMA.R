#Simulation code for HDMA with 1000 replications
library(hdmed)
Sim_ms<-function(Q){
  set.seed(1e3+Q)
  n <- 100  # sample size
  ...
  HDMA.fit<-mediate_hdma(dat$X, dat$M, dat$Y, dat$Z, dat$Z)
  ID_ms_L0<-rep(0,p)
  me_est<-rep(0,p)
  ID_true<-as.numeric((A*B)!=0)
  if(!is.null(HDMA.fit)){
    HDMA.fit2<-HDMA.fit$contributions[which(HDMA.fit$contributions[,7]<0.05),]
    HDMA.fit2[,1]<-as.numeric(substr(HDMA.fit2[,1], 2, 1000))
    ID_ms_L0[HDMA.fit2[,1]]=1
    me_est[HDMA.fit2[,1]]=HDMA.fit2[,2]*HDMA.fit2[,4]
  }
...
clusterExport(cl, c('simL0hima','L0hima','mediate_hdma'))
...
R_HDMA_1<-round(100*c(FWER,TPR,FNR,FPR,TNR,PPV,FOR,FDR,NPV,F1_score),2)
names(R_HDMA_1)<-c("FWER","TPR","FNR","FPR","TNR","PPV","FOR","FDR","NPV","F1_score")
# R_HDMA_1
sel<-cbind((apply(res1[1:pnumber,],1,mean)))
R_HDMA_2<-sel
R_HDMA_2<-round(100*R_HDMA_2,3)
...
R_HDMA_3[c(1:10,pnumber+1),c(2,3,4)]